...
resource "aws_ecs_task_definition" "user_service" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "POSTGRES_DB",       value = "uitgo" }, # Tên DB chung

      # JWT Secret (Nên dùng AWS Secrets Manager trong thực tế, ở đây demo dùng biến)
      { name = "JWT_SECRET",        value = "supersecretkey_production" }
    ]
...
resource "aws_ecs_task_definition" "trip_service" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      # 5. Security
      { name = "JWT_SECRET",          value = "supersecretkey_production_change_this" },
      { name = "JWT_EXPIRE_HOURS",    value = "24" }
    ]
    logConfiguration = {
      logDriver = "awslogs"
